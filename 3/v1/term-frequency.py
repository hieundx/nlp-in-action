...
# splits text into words base on Penn Bank Rules
tokenizer = TreebankWordTokenizer()

tokens = tokenizer.tokenize(kite_text.lower())
print('10 tokens (including stopwords)')
pprint(tokens[:10])

token_counts = Counter(tokens)
print('Top 10 token counts (including stopwords)')
...
print('10 tokens (excluding stopwords)')
pprint(tokens_no_stopwords[:10])

kite_counts = Counter(tokens_no_stopwords)
print('Top 10 token counts (excluding stopwords)')
pprint(kite_counts.most_common(10))

print('Term frequency (TF) for 10 most common tokens')
tf_kite = {token: round(count/len(tokens_no_stopwords), 4) for token, count in kite_counts.items()}
sorted_tf_kite = sorted(tf_kite.items(), key=lambda x: x[1], reverse=True)
pprint(sorted_tf_kite[:10])